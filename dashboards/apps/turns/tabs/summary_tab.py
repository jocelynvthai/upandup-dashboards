...
#         )
#         turn_chargebacks_clawbacks_df[f'{time_granularity_col}_str'] = pd.to_datetime(turn_chargebacks_clawbacks_df['amortized_date']).apply(
#             lambda d: d + relativedelta(days=6 - d.dayofweek) if pd.notnull(d) else pd.NaT
#         ).dt.strftime('%Y-%m-%d')
#         # cutoff date and add empty weeks
...
#     # add turn chargebacks and clawbacks
#     st.dataframe(turn_chargebacks_clawbacks_df)
#     grouped_turn_chargebacks_clawbacks_df = turn_chargebacks_clawbacks_df.groupby(f'{time_granularity_col}_str').agg({
#         'amount': 'sum',
#     }).rename(columns={'amount': 'turn_chargebacks_clawbacks'}).reset_index()
#     grouped_df = grouped_df.merge(grouped_turn_chargebacks_clawbacks_df, on=f'{time_granularity_col}_str', how='left').fillna(0)
...
#         if cost_type == 'total':
#             value_vars = ['project_total_estimated_cost', 'project_invoiced_cost', 'turn_chargebacks_clawbacks', '# Turns']
#         else:
#             value_vars = ['project_total_estimated_cost_per_turn', 'project_invoiced_cost_per_turn', 'turn_chargebacks_clawbacks_per_turn', '# Turns']
...
#         melted_df['cost_type'] = melted_df['cost_type'].map({value_vars[0]: 'Estimated Cost', value_vars[1]: 'Invoiced Cost', value_vars[2]: 'Chargebacks/Clawbacks', value_vars[3]: '# Turns'})
...
#                 scale=alt.Scale(
#                     domain=['Estimated Cost', 'Invoiced Cost', 'Chargebacks/Clawbacks', '# Turns'],
#                     range=[LIGHT_GRAY, DARK_TEAL, TEAL, GRAY]
#                 )
#             ), 
#             xOffset='cost_type:N',
#             tooltip=[
