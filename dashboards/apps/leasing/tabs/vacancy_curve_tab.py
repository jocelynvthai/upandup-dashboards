...
def vacancy_curve_filters(distinct_vacancies_df, vacancy_curve_df):
    ...
    filtered_vacancy_curve_df = vacancy_curve_df[
        ...
        (vacancy_curve_df['vacancy_start_date'].astype(str) == selected_vacancy[1])
    ]
    return filtered_vacancy_curve_df, selected_vacancy


def vacancy_curve(filtered_vacancy_curve_df):
    col1, col2, col3, col4, col5 = st.columns(5)
    with col1:
        st.metric("Total # Inquiries", filtered_vacancy_curve_df['cumulative_num_inquiries'].max())
    with col2:
        st.metric("Total # Tours", filtered_vacancy_curve_df['cumulative_num_tours'].max())
    with col3:
        st.metric("Total # Applications", filtered_vacancy_curve_df['cumulative_num_applications'].max())
    with col4:
        st.metric("# Days Vacant", filtered_vacancy_curve_df['days_since_vacancy_start'].max())
    with col5:
        st.metric("# Days Listed", filtered_vacancy_curve_df['is_internally_listed'].sum())

    
    # 1. is_internally_listed background
    background = alt.Chart(filtered_vacancy_curve_df).mark_bar(
        opacity=0.3,
        width=10,
        color=LIGHT_GRAY
    ).encode(
    ...
    background_data = filtered_vacancy_curve_df[filtered_vacancy_curve_df['is_internally_listed']][['days_since_vacancy_start']]
    background_data['metric'] = 'Listing Window'
    background_data['count'] = None 
    listed_rent_data = filtered_vacancy_curve_df[['days_since_vacancy_start']]
    ...
    rent_chart = alt.Chart(filtered_vacancy_curve_df).mark_line(point={'color': TEAL}, color=TEAL).encode(
        x=alt.X('days_since_vacancy_start:Q', title='Days Since Vacancy Start'),
