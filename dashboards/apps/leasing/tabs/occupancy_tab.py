...
def economic_occupancy(economic_occupancy_df):
    ...
    with st.container(border=True):
        ...
        st.markdown(
            '<div style="text-align: right;"><em>Best case assumes that all current leases will be renewed, whereas the worst case assumes that all current leases will move out.</em></div>',
...
def num_leases_to_target_3(economic_occupancy_df):
    ...
    with st.container(border=True):
        ...
        for index, row in target_leases.iterrows():
            ...
            new_gpr_needed_to_hit_budget = max(deadline_row['total_gpr_occupied_budget'] - catch_up_gpr, 0)
