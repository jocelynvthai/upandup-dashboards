...
def all_management_expenses_data(_credentials, start_date, end_date):
    query = f"""
        SELECT
            ...
            ame.category,
            ame.subcategory,
            ame.additional_category, 
            ame.maintenance_category,
            ame.maintenance_subcategory,
            ame.amount,
            ame.description,
...
@st.cache_data(ttl=CACHE_TTL)
def imputed_daily_budget_data(_credentials, start_date):
    query = f"""
        WITH dates AS (
            SELECT
                *,
            ...
            WHERE _date BETWEEN '{start_date}' AND CURRENT_DATE
        )
