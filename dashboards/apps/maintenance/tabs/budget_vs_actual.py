...
def budget_vs_actual_filters(bills_tickets_invoices_df):
    ...
    with col_date_range:
        date_range = st.date_input("Pick a period range", 
                            ...
                            key='budget_vs_actual_date_range')
        if len(date_range) != 2:
            st.stop()
        else:
...
def budget_vs_actual(bills_tickets_invoices_df):
    ...
    selected_time_granularity = st.selectbox("Select a time granularity", time_granularity_dict.keys(), key='budget_vs_actual_time_granularity')
...
def budget_vs_actual_line_items(bills_tickets_invoices_df):
    st.subheader("Line Items")
    line_items_df = bills_tickets_invoices_df.copy()
    columns = [
        # 'id', 
        'date', 
        'properties', 
