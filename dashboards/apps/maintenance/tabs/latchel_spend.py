...
def latchel_spend_filters(credentials):
    # filters
    ...
    with col_date_range:
        ...
        if len(date_range) != 2:
            ...
            filtered_bills_tickets_invoices_df = latchel_spend_data_clean(bills_tickets_invoices_df)
    with col_vendor:
...
def latchel_spend(bills_tickets_invoices_df):
    st.subheader("Latchel Spend Over Time (Budget vs Actual)")
    ...
    selected_time_granularity = st.selectbox("Select a time granularity", time_granularity_dict.keys(), key='latchel_spend_time_granularity')
    ...
    grouped_bills_tickets_invoices_df = (
        ...
        .agg(buildium_bill_spend=('total_bill_amount', 'sum'), latchel_invoice_amount=('invoice_total_amount', 'sum'), latchel_ticket_budget=('ticket_max_cost', 'sum'))
        .reset_index()
    )
...
def latchel_spend_bills(bills_tickets_invoices_df):
    st.subheader("Latchel Bills & Journal Entries")
    ...
    styled_df = (
        ...
        .style.apply(
            lambda df: pd.DataFrame(
                np.select(
                    [
                        df['ticket_max_cost'].isna(), 
                        (df['total_bill_amount'] > df['ticket_max_cost']) & df['ticket_max_cost'].notna(),
                        (df['total_bill_amount'] > df['invoice_total_amount']) & df['invoice_total_amount'].notna(),
                    ],
                    [
                        ['color: red;'] * len(df),
                        ['color: orange;'] * len(df),
    ...
    st.caption(
        ...
        <b>Buildium bill spend:</b><br>
        🔴 No Latchel ticket budget 🟠 Exceeds Latchel ticket budget 🟡 Exceeds Latchel invoice amount
        </p>
        """,
