...
from tabs.seasonality_tab import seasonality_filters, buildium_spend_seasonality, seasonality_by_category
from tabs.buildium_spend_tab import buildium_spend_filters, buildium_spend_per_home, buildium_spend_over_time, buildium_spend_line_items
...
with buildium_spend_tab:
    ...
    if selected_tab == 'Overall':
        filtered_all_management_expenses_df, filtered_owned_homes_df, filtered_imputed_daily_budget_df = buildium_spend_filters(credentials)
        buildium_spend_per_home(filtered_all_management_expenses_df, filtered_owned_homes_df, filtered_imputed_daily_budget_df)
    ...
    if selected_tab == 'Seasonality':
        filtered_all_management_expenses_df, filtered_owned_homes_df, filtered_budget_by_month_df = seasonality_filters(credentials)
